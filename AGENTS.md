...
6. `requesting-code-review` — the first suggested `Next` action. It reviews the diff between `HEAD` and a user-supplied fixed point with two fresh reviewers in parallel: a Standards axis for documented conventions and code smells, and a Spec axis for explicit requirement compliance. Both sub-400-word reports stay side by side; they are not merged or reranked. The skill owns neither fixes nor loops.
7. `llm-md-revise` — the second suggested `Next` action, after review and any accepted corrections. It filters the session and optional branch diff for durable project knowledge, follows the repository's existing canonical instruction surface and thin imports, presents all surviving diffs together, and applies only the user's selected set. If no candidates survive, it reports that and stops. It never writes user-scope files or persists secrets and PII.
