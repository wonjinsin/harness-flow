...
4. `implement` — 단일 코드 변경 controller입니다. Caller는 agreed brief, approved plan, confirmed bug-fix brief를 동일한 settled implementation input으로 정규화해 넘깁니다. `implement`는 현재 체크아웃에서 TDD로 직접 구현하고 task 또는 완료된 brief마다 커밋합니다. 첫 수정 전에 dirty checkout이면 멈추고, 불변 `BASE_SHA`, base branch, baseline test를 확인합니다. 구현과 리뷰 중 branch/worktree를 만들거나 바꾸지 않습니다. 선택적 task isolation과 종료 절차는 필요한 시점에만 내부 reference를 읽습니다. Completeness와 durable candidate 정리가 끝나면 최초 `BASE_SHA..HEAD`를 fresh-context, report-only reviewer에게 보냅니다. Blocker 수정은 일괄 처리하고 테스트·커밋한 뒤 `LAST_REVIEWED_SHA..HEAD` 증분 범위를 이전 보고서와 함께 새 reviewer에게 보냅니다. 수정 리뷰는 최대 두 번입니다. 실패한 bug-fix attempt는 일반 revert commit으로 제거한 뒤 `systematic-debugging`에 새 증거를 돌려줍니다.
...
6. `llm-md-revise` — durable candidate가 있을 때 review range를 고정하기 전에 실행합니다. 사용자 correction, always/never rule, project fact, anti-pattern, external-system reference를 후보로 제시하고, 승인된 diff만 active harness의 project instruction surface(Codex → `AGENTS.md`, Claude Code → `CLAUDE.md`, `@import`가 있으면 실제 파일) 또는 project `rules/*.md`에 적용합니다. 승인된 edit는 review range에 포함되도록 먼저 commit해야 합니다. User-scope file은 중복 확인용으로만 읽고 쓰지 않습니다.
7. `requesting-code-review` — 호출마다 **mid-tier model**의 fresh-context `general-purpose` reviewer 한 번을 dispatch하고 report를 반환합니다. Fix나 loop를 소유하지 않습니다. 입력은 inline requirements, 정확한 `FROM_SHA`, `TO_SHA`, 이전 complete report 원문(최대 두 개)입니다. Caller가 준 range는 재계산하지 않으며 현재 `HEAD`가 `TO_SHA`와 다르면 중단합니다. Reviewer는 changed-file diff를 파일별로 읽고 `N/N` coverage를 증명합니다. 결과 판단값은 `Review complete: yes | no`와 `Blocking findings: none | finding list`뿐입니다. Native read-only control이 없으면 bounded before/after snapshot으로 나열된 repository-state change를 탐지하지만 ignored-file content와 fail-closed isolation은 보장하지 않습니다.

Review가 통과하면 `implement`는 `APPROVED_SHA`를 고정하고 그때만 내부 종료 reference를 읽습니다. Clean state와 `HEAD == APPROVED_SHA`를 다시 확인한 뒤 PR 또는 감지된 base branch(`origin` default, `main`, `master` 순서) merge를 묻습니다. PR 경로는 같은 SHA를 `pr-creator`에 전달하고, publish 직전 local HEAD와 remote branch tip, 생성 후 PR의 `headRefOid`를 확인합니다. Merge 경로는 사용자가 명시적으로 고른 local merge만 수행합니다. 어느 경로도 기존 branch나 외부 생성 worktree를 자동 삭제하지 않습니다.
