...
**요청 유형별 routing(등급 분류 없음).** `using-harness-flow`는 첫 일치 규칙만 적용한다. 승인된 plan·합의된 brief는 `implement`, 승인된 spec은 `writing-plans`로 보낸다. Skill-only 생성·편집·검증은 일반 read-only 분석보다 우선하여 code-mutation chain 밖의 `writing-skills`로 직접 보낸다. 아직 원인이 확인되지 않은 bug·test failure·unexpected behavior는 명시적 구현 plan 요청이 함께 있어도 `systematic-debugging`이 우선한다. 원인 확인 뒤 사용자가 plan을 요청했다면 confirmed bug-fix brief를 `writing-plans`로, 아니면 `implement`로 보낸다. 명시적 code review는 `requesting-code-review`, 명시적 spec은 `brainstorming` explicit-spec mode, 비버그의 명시적 implementation plan은 `writing-plans`로 보낸다. 그 밖의 feature/refactor/script/change-intent와 범위 내 codebase·repository·technical artifact의 read-only 조사·분석·보고는 `brainstorming`으로 보낸다. General-knowledge questions stay outside the chain. **Dual-mode principle:** every chain skill is also independently invocable — the chain is the default route, and a skill's preconditions are guards, not gates: invoked without its usual input, the skill recovers it (e.g. `writing-plans` asks the 1–2 settling questions first) rather than bouncing the user back through the chain.
