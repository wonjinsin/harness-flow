...
7. `requesting-code-review` — dispatch a `general-purpose` code reviewer subagent (template at `requesting-code-review/code-reviewer.md`) on a **mid-tier model** (2026-08-06 eval: sonnet + severity floor caught 6/6 discovery-class defects on 700–900-line diffs, 0 demotions — top-tier premium unneeded; template also carries per-finding/Strengths format caps with an explicit no-cap-on-finding-count rule). Used by `implement`'s final whole-branch review and by `brainstorming`'s small exit when the finished diff is non-trivial. The template's Calibration carries a **severity floor** (rate by consequence, not surface form) to counter demotion of consequential findings at the single final review.
