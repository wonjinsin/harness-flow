...
const claudeMarketplace = JSON.parse(
  fs.readFileSync(path.join(ROOT, '.claude-plugin/marketplace.json'), 'utf-8')
);
...
test('Claude Code and Codex publish version 2.3.3', () => {
  assert.equal(claude.version, '2.3.3');
  assert.equal(codex.version, '2.3.3');
});
