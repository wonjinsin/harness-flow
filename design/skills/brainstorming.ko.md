...
`harness-flow.yaml` 기준 라우팅 (`next` 는 **바로 다음** 하위 노드 — 메인 스레드가 각 writer 완료 후 나머지를 재도출):

- `prd-trd` → `next: "prd-writer"` (전체 체인: prd-writer → trd-writer → task-writer)
- `prd-only` → `next: "prd-writer"` (전체 체인: prd-writer → task-writer)
- `trd-only` → `next: "trd-writer"` (전체 체인: trd-writer → task-writer)
