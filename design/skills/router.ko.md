...
최종 JSON 을 emit 하기 전에 `using-harness § Core loop` 의 3–5 단계에 따라 `harness-flow.yaml` 의 자기 자신의 outgoing edge 에 대해 next-node 조회를 수행한다. 결정 자체는 단순하다 — 위 표가 각 outcome 을 `next` 로 매핑한다:

- `casual` → 매칭되는 downstream edge 없음 → `next: null`
- `clarify` / `plan` / `resume` → `next: "brainstorming"`

emit 하는 JSON 에 `next` 를 필드로 포함한다. `casual` 인 경우 이 단계를 건너뛴다 (애초에 JSON 을 emit 하지 않음).
...
- `next`: Step 6 에서 결정된 downstream node id — 위 outcome 들에서는 항상 `"brainstorming"`
