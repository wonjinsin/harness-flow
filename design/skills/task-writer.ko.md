...
`prd_path` 가 세팅돼 있는데 파일을 못 읽으면 중단하고 `{"outcome": "error", "session_id": "...", "reason": "PRD declared in payload but <path> not found", "next": null}` emit. `trd_path` 도 동일. 지어내서 진행하지 말 것.

`prd_path` · `trd_path` · `brainstorming_output` 이 모두 null 이고 `request` 가 actionable verb 없는 한 문장 (예: "좋네요") 이면 `{"outcome": "error", "session_id": "...", "reason": "insufficient input to derive tasks", "next": null}` emit. 메인 스레드가 오라우팅한 가능성이 높음; recovery 는 메인 스레드 결정.

## Output

최종 메시지는 항상 `outcome` 태그가 붙은 JSON 객체 하나. `next` 필드는 아래 Step 6 에서 결정된다.
...
예산 cap 에 도달했는데 위 세 질문에 답할 수 없으면 중단하고 `{"outcome": "error", "session_id": "...", "reason": "codebase exploration exhausted budget without resolving change surface", "next": null}` emit. 상류 payload 가 덜 명세된 상태일 가능성이 높음 — 상류 writer 재-dispatch 여부는 메인 스레드 결정.
...
파일이 이미 있으면 중단하고 `{"outcome": "error", "session_id": "...", "reason": "TASKS.md already exists at <path>", "next": null}` emit. 재생성은 메인 스레드 몫.
...
이 스킬의 `harness-flow.yaml` 내 outgoing edge 에 대해 `using-harness § Core loop` 단계 3–5 를 따라 next-node lookup 을 수행한다. 후보는 `executor` 단 하나 (`depends_on: [task-writer]`, `when:` 없음):
...
Task 개수를 센다. 결정된 `next` 와 함께 최종 JSON 을 emit. 이게 최종 메시지 전부.
...
- **DAG 에 사이클**: 파일을 쓰지 말 것. `{"outcome": "error", "session_id": "...", "reason": "task DAG contains cycle: task-N → task-M → task-N", "next": null}` emit. 재분해 여부는 메인 스레드 결정.
