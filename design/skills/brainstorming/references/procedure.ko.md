...
- 2–3 개 방향성 MC — `AskUserQuestion` 으로 `header: "Direction"` 사용, 상위 *모양 카테고리*만, 구현 NOT:
  - ✓ "알림 시스템이라면 — 푸시 / 이메일 / 인앱 중에 어디?" → `AskUserQuestion` 으로 3개 옵션
  - ✓ "이메일 자동화라면 — 초안만 보조하는 건지, 자동 발송까지 가는 건지?" → `AskUserQuestion` 으로 2개 옵션
  - ✗ "Pub/Sub vs cron vs polling 중에?" — 구현 선택, prd-writer / trd-writer 의 몫.
  - ✗ "Postgres 와 MongoDB 중에 어디?" — 동일.
  - 정식 포맷은 `../../harness-contracts/ask-user-question.ko.md` § "Brainstorming — explore direction mapping" 참조.
...
1. **intent** — 보통 추론 가능. 진짜 애매할 때만: `AskUserQuestion` 으로 정식 intent 옵션 제시. `../../harness-contracts/ask-user-question.ko.md` § "Brainstorming — intent" 참조. 유저의 동사가 옵션 중 어디에도 안 맞으면 `intent: "other"` 로 두고 **함께** `constraints` 에 `"intent-freeform: <동사>"` 를 추가 — Phase B 가 원 동사를 볼 수 있어야 한다.
2. **target** — "코드베이스의 어느 부분에 해당하나요?" 열린 질문. A1.6 에서 후보가 보이면 `AskUserQuestion` (최대 4 옵션).
3. **scope_hint** — `AskUserQuestion` 으로 정식 scope 옵션 제시. `../../harness-contracts/ask-user-question.ko.md` § "Brainstorming — scope" 참조.
...
필수 체크리스트가 다 찼으면 유저 언어로 짧은 산문 요약을 먼저 보내고, 이어서 `AskUserQuestion` 으로 확인을 받는다:

> "확인 — {intent} {target}, {scope_hint}. {constraint 요약이 있다면}. {acceptance 가 있다면}."
> [이어서: "맞아요" / "수정할게 있어요" 옵션으로 AskUserQuestion 호출]

정식 옵션 목록은 `../../harness-contracts/ask-user-question.ko.md` § "Brainstorming — confirm fills (A4)" 참조. 경로 추천은 이 메시지에 묶지 않는다.

`AskUserQuestion` 응답 (또는 **다음** 유저 턴) 에서:

- "맞아요" / "Looks good" / 무응답 → Phase B 로 진입 (B1 부터).
- "수정할게 있어요" / Other 로 정정 → A2 의 **그 필드만** 으로 돌아가서 재확인. 수정 ≠ 재시작; 이미 옳게 답한 필드는 다시 묻지 않는다.
...
유저 언어로 산문 요약을 먼저 보내고, 이어서 `AskUserQuestion` 을 호출한다:

> "**{route}** ({expansion}) 추천해요. 예상 파일 수 {N}개. {신호 요약 또는 '보안·아키텍처 신호 없음.'}"
> [이어서: "진행" / "루트 변경" / "파일 수 조정" 옵션으로 AskUserQuestion 호출]

정식 옵션 목록은 `../../harness-contracts/ask-user-question.ko.md` § "Brainstorming — Gate 1" 참조. 터미널 상태 블록은 이 메시지에 붙이지 말 것. AskUserQuestion 응답 대기.
