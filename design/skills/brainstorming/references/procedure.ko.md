...
intent + target 이 잡힌 시점에 한 번 돌린다. 요청에 해결 가능한 target 이 없을 때만 (순수 UX 결정, 로컬 아날로그가 없는 신규 외부 통합) 스킵 — `exploration_findings: null` 로 A2 진행.
...
산출: 작업 메모리에 보유하는 `exploration_findings` 초안, B7 에서 확정·emit:
...
`code_signals` 는 경로 패턴 + 코드가 가시적으로 관여하는 개념 신호 (auth/login/schema/migration/config/dependency) 둘 다 나열. 여기 `open_questions` = **사용자**가 A2 또는 Gate 1 에서 답해야 할 것 — PRD/TRD/TASKS 의 Open questions (사람 검토용) 와 구분.
...
`exploration_findings.open_questions` 항목 중 블로킹인 것은 A2 질문으로 승격 — 사용자는 이를 해결할 가장 싼 자리다.
...
**(a) 경로 신호 — 리터럴, 언어 무관.** `request`, `target`, `constraints`, `exploration_findings.code_signals` 에서 다음 파일 경로 패턴을 스캔:
...
정수 하나 N — 수정 + 신규 파일의 베스트-게스. `exploration_findings.files_visited` 가 있으면 floor 로 활용; 아직 방문하지 않은 호출자/테스트만큼 외삽.
...
출력 JSON 같이 붙이지 말 것. MC 암묵 제시 (수락 / 경로 변경 / 파일 수 조정). 다음 턴 대기.
...
- **피벗 또는 casual** — `{"outcome": "pivot", ...}` emit ("새 요청으로 보입니다; 라우팅으로 돌아갑니다.") 또는 `{"outcome": "exit-casual", ...}` (작업 요청이 아닌 질문이었음). 피벗 시 ROADMAP/STATE 갱신 금지.
...
3. **경로 payload 를 마지막 메시지로 emit** — `outcome` 은 경로 이름 (`prd-trd` / `prd-only` / `trd-only` / `tasks-only` / `pivot` / `exit-casual`). A1.6 가 돌았으면 payload 에 `exploration_findings` 포함 (그렇지 않으면 `null`). 메인 스레드가 SKILL.md 의 "Required next skill" 섹션을 읽어 맞는 writer 로 dispatch.
