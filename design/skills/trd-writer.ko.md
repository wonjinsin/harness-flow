...
- `brainstorming_outcome`: brainstorming 이 emit 한 route (`"prd-trd"` 또는 `"trd-only"`). 필수 — Step 5 에서 하류 `when:` 평가에 사용해 `next` 를 결정한다. 없거나 다른 값이면 `error` emit (상류 계약 위반).
...
`prd_path` 가 세팅돼 있는데 파일을 못 읽거나 존재하지 않으면 즉시 중단하고 `{"outcome": "error", "session_id": "...", "reason": "PRD declared in payload but <path> not found", "next": null}` 를 emit. 지어내서 진행하지 말 것 — payload 는 메인 스레드와의 계약이다.

## Output

최종 메시지는 항상 `outcome` 태그가 붙은 JSON 객체 하나. `next` 필드는 아래 Step 5 에서 결정된다.
...
`done` 일 때 `next` 는 항상 `"task-writer"` — 도출 과정은 Step 5 참조.
...
파일이 이미 있으면 중단하고 `{"outcome": "error", "session_id": "...", "reason": "TRD.md already exists at <path>", "next": null}` emit. 재생성은 메인 스레드 몫 — 이전 파일을 먼저 지우고 재-dispatch.

### Step 5 — Resolve `next` and emit

`using-harness § Core loop` 단계 3–5 에 따라 `harness-flow.yaml` 에서 이 스킬의 outgoing edge 에 대해 next-node 조회를 수행한다. 유일한 후보: `task-writer` (`depends_on` 에 `trd-writer` 포함). `$brainstorming.output.outcome` 을 `brainstorming_outcome` 필드로 치환해 `task-writer` 의 `when:` 을 평가:
...
(`done` 행이 균일한 이유: `trd-writer` 는 `brainstorming_outcome` 이 `prd-trd` 또는 `trd-only` 일 때만 dispatch 되며, 둘 다 `task-writer` 의 `when:` 집합에 포함된다.)

"Open questions" 항목 개수를 센다. 결정된 `next` 와 함께 최종 JSON 을 emit. 이게 최종 메시지 전부.
