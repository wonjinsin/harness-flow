...
- `brainstorming_outcome`: brainstorming 이 emit 한 라우트 (`"prd-trd"` 또는 `"prd-only"`). 필수 — Step 5 에서 하위 `when:` 식을 평가해 `next` 를 결정하는 데 쓰인다. 누락되거나 다른 값이면 `error` emit (상위 계약 위반).
...
최종 메시지는 항상 `outcome` 태그 JSON 하나. `next` 필드는 아래 Step 5 에서 결정된다.
...
`next` 는 `brainstorming_outcome == "prd-trd"` 일 때 `"trd-writer"`, `brainstorming_outcome == "prd-only"` 일 때 `"task-writer"`. Step 5 참조.
...
파일이 이미 존재하면 중단하고 `{"outcome": "error", "session_id": "...", "reason": "PRD.md already exists at <path>", "next": null}` emit. 재생성은 메인 스레드 몫 — 이전 파일을 먼저 지우고 재 dispatch.

### Step 5 — Resolve `next` and emit

`using-harness § Core loop` 의 step 3–5 절차에 따라 이 스킬의 outgoing edge 들에 대해 next-node 조회를 수행한다. 후보: `trd-writer`, `task-writer` (둘 다 `depends_on` 에 `prd-writer` 포함, 둘 다 `trigger_rule: one_success`). `$brainstorming.output.outcome` 자리에 payload 의 `brainstorming_outcome` 필드를 대입한 뒤, `harness-flow.yaml` 순서대로 각 후보의 `when:` 을 평가한다:
...
"Open questions" 항목 수 카운트. 결정된 `next` 와 함께 최종 JSON emit. 이게 최종 메시지의 전부.
